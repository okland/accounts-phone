...
if (Meteor.isClient) (function () {
    ...
    var loggedInAs = function (somePhone, test, expect) {
        return expect(function (error) {
            test.equal(error, undefined);
            var user = Meteor.user();
            test.equal(user.phone.number, somePhone);
        });
    };
    ...
    testAsyncMulti("phones - basic login with password", [
        ...
        logoutStep,
        function (test, expect) {
            Meteor.loginWithPhoneAndPassword(this.phone, this.password,
                loggedInAs(this.phone, test, expect));
        },
        logoutStep,
        ...
        function (test, expect) {
            ...
            var handle = Tracker.autorun(function () {
                if (Meteor.user() && Meteor.user().phone)
                    loaded = true;
            });
            // At the beginning, we're not logged in.
            test.isFalse(loaded);
            Meteor.loginWithPhoneAndPassword(this.phone, this.password, expect(function (error) {
                test.equal(error, undefined);
                test.notEqual(Meteor.userId(), null);
                // By the time of the login callback, the user should be loaded.
                test.isTrue(Meteor.user().phone);
...
//        function (test, expect) {
//            // setup
//            this.phone = '+97254580'+ Math.floor(Math.random() * 1000 - 1000) + 1000;
//            this.password = 'password';
